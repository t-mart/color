...
<button on:click={(e) => copyToClipboard(color, e)} class="flex flex-col items-center relative gap-y-2 hover:underline">
	<div style="background-color: {hashColor}" class="h-10 w-10 rounded"></div>
	<p class="font-mono text-xs">{hashColor}</p>
    {#if copyTooltipVisible}
        <div class="absolute bottom-full mb-2" transition:fade={{ duration: 200 }}>
